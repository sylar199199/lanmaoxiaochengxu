...
Page({
  ...
  getrec(){
    wx.showLoading()
    util.request('lanmao/community/user/rec/follows', 'POST', '', '', (res) => {
  ...
  getList: function () {
    ...
    util.request('lanmao/community/user/follows', 'POST', data, '', (res) => {
      wx.hideLoading()
      if (res.data.errorCode == 5001) {//重新获取code,弹框授权获取用户信息
  ...
  cancelcare(e){
    var that = this;
    if(e){
      var isCare = e.currentTarget.dataset.isCare;
        ...
        if (res.code) {
          ...
          util.request('lanmao/mp/user/login', 'POST', data, '', (loginres) => {
            ...
            if (loginres.data.errorCode == 5010) {//重新获取code,弹框授权获取用户信息
              ...
              console.log(source == 'getrec')
              wx.setStorageSync('token', loginres.data.data.token)
              if(source == 'getrec'){
                console.log(source)
                that.getrec();
              }else if(source == 'getList')
                that.getList();
            }else if(source == 'care'){
              that.care();
